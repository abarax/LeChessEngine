...
//  LeChessEngine
//
//  Created by Leigh Appel on 17/12/2015.
...
func perft(var board:Board, depth:Int) -> Int {
    ...
    for move in moves {
        ...
        result += perft(board, depth: depth - 1)
...
func divide(var board:Board, depth:Int){
    ...
    for move in moves {
        result = 0
        let undo = doMove(&board, move: move)
        result += subdivide(board, depth: depth - 1)
        undoMove(&board, undo: undo)
        print("\(move.from) \(move.to) (\(move.promotion)) - \(result)")
    }
}

func subdivide(var board:Board, depth:Int) -> Int {
    if (!board.isLegal()) {
        return 0
    }
...
func runPerftTest() {
    var board = Board()
    for (fen, expectedValues) in perftData {
        print("Running tests for \(fen)")
        board.loadFenBoardState(fen)
        for (depth, expected) in expectedValues.enumerate() {
            print("Running depth \(depth + 1)")
            
            if expected == perft(board, depth: depth + 1) {
                print("SUCCESS")
            } else {
                print("Perft Test FAILED for Fen(\(fen)) at depth (\(depth + 1)")
            }
        }
        board.clear()
    }
